<?php

namespace common\models;

use Yii;
use yii\base\Model;
use yii\data\ActiveDataProvider;
use common\models\Payouts;

/**
 * PayoutsSearch represents the model behind the search form about `\common\models\Payouts`.
 */
class PayoutsSearch extends Payouts
{
    /**
     * @inheritdoc
     */
    public function rules()
    {
        return [
            [['id', 'start_block', 'end_block', 'amount'], 'integer'],
            [['tx_hash', 'paid_on'], 'safe'],
        ];
    }

    /**
     * @inheritdoc
     */
    public function scenarios()
    {
        // bypass scenarios() implementation in the parent class
        return Model::scenarios();
    }

    /**
     * Creates data provider instance with search query applied
     *
     * @param array $params
     *
     * @return ActiveDataProvider
     */
    public function search($params)
    {
        $query = Payouts::find();

        // add conditions that should always apply here

        $dataProvider = new ActiveDataProvider([
            'query' => $query,
        ]);

        $this->load($params);

        if (!$this->validate()) {
            // uncomment the following line if you do not want to return any records when validation fails
            // $query->where('0=1');
            return $dataProvider;
        }

        // grid filtering conditions
        $query->andFilterWhere([
            'id' => $this->id,
            'start_block' => $this->start_block,
            'end_block' => $this->end_block,
            'amount' => $this->amount,
            'paid_on' => $this->paid_on,
        ]);

        $query->andFilterWhere(['like', 'tx_hash', $this->tx_hash]);

        return $dataProvider;
    }
}
